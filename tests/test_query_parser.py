...
class TestParseQueryTitleRefs:
    ...
    def test_more_like(self):
        # "more like X" — generated by the UI "More like this" button
        p = parse_query("more like Inception")
        assert "Inception" in p.reference_titles
        assert "Inception" not in p.person_names

    def test_more_movies_like(self):
        # "more movies like X" with multi-word title
        p = parse_query("more movies like The Shawshank Redemption")
        assert "The Shawshank Redemption" in p.reference_titles

    def test_more_films_like_with_year(self):
        # UI generates "More movies like Title (Year)" — the lookahead stops at "("
        p = parse_query("more movies like Parasite (2019)")
        assert "Parasite" in p.reference_titles
