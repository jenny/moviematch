...
_TITLE_REF_PATTERNS: list[tuple[re.Pattern, bool]] = [
    ...
    # "more like Inception" / "more movies like Inception" / "more films like Inception"
    # Generated by the "More like this" UI button; title-only, no person promotion.
    (re.compile(
        r"\bmore(?:\s+(?:movies?|films?))?\s+like\s+[\"']?([A-Z][A-Za-z0-9\s:!?'&,-]{1,50}?)[\"']?"
        r"(?=\s+but|\s+only|\s+with|\s*[,.(]|$)",
        re.IGNORECASE,
    ), False),
